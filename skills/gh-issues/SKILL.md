...
For each confirmed issue, spawn a sub-agent using sessions\_\_spawn. Launch up to 8 concurrently (matching `subagents.maxConcurrent: 8`). If more than 8 issues, batch them — launch the next agent as each completes.
...
For each issue, construct the following prompt and pass it to sessions\_\_spawn. Variables to inject into the template:
